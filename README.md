...
**نیازی به پاک کردن دستی دیتابیس نیست.** اگر جداول از نصب قبلی وجود داشته باشند، نصب خودکار همه داده‌ها را پاک و دوباره از `install-bundle.json` بارگذاری می‌کند.
...
| `*.xlsx` در ریشه پروژه | فقط برای ساخت bundle بودند |
| `tools/` | اسکریپت توسعه، روی سرور لازم نیست |

| نگه دارید | دلیل |
|-----------|------|
| `config.php` | اتصال دیتابیس و ورود |
| `data/install-bundle.json` | بازنشانی داده از دکمه داخل پنل |
